...
$GLOBALS['TL_DCA']['tl_gallery_creator_albums'] = array(
    ...
    'palettes' => array(
           ...
           'default' => '{album_info},published,name,alias,assignedDir,album_info,displ_alb_in_this_ce,owner,date,event_location,sortBy,thumb,comment,visitors;{insert_article},insert_article_pre,insert_article_post;{protection:hide},protected',
...
class tl_gallery_creator_albums extends Backend
{
    ...
    public function buttonCbAddImages($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        return '<a href="' . $this->addToUrl($href) . '" title="' . specialchars($title) . '"' . $attributes . ' style="margin-right:5px">' . Image::getHtml($icon,
               $label) . '</a>';
    }
    ...
    public function toggleVisibility($intId, $blnVisible)
    {
        ...
        if (!$this->User->isAdmin && $objAlbum->owner != $this->User->id && !$GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection']) {
            $this->log('Not enough permissions to publish/unpublish tl_gallery_creator_albums ID "' . $intId . '"',
                   __METHOD__, TL_ERROR);
        ...
        $objVersions->create();
        $this->log('A new version of record "tl_gallery_creator_albums.id=' . $intId . '" has been created.',
    ...
    public function buttonCbCutPicture($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        return (($this->User->id == $objAlb->owner || $this->User->isAdmin || true === $GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection']) ? ' <a href="' . $this->addToUrl($href . '&id=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
    ...
    public function buttonCbDelete($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        $objAlb = $this->Database->prepare('SELECT owner FROM tl_gallery_creator_albums WHERE id=?')->execute($row['id']);
        return ($this->User->isAdmin || $this->User->id == $objAlb->owner || true === $GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection']) ? '<a href="' . $this->addToUrl($href . '&id=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
                      $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';
    }
    ...
    {
        return '<a href="' . $this->addToUrl($href . '&id=' . $row['id'],
               1) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
               $label) . '</a> ';
    }
    ...
    public function buttonCbPastePicture(DataContainer $dc, $row, $table, $cr, $arrClipboard = false)
    {
        ...
        $imagePasteAfter = Image::getHtml('pasteafter.gif',
               sprintf($GLOBALS['TL_LANG'][$table]['pasteafter'][1], $row['id']), 'class="blink"');
        $imagePasteInto = Image::getHtml('pasteinto.gif',
               sprintf($GLOBALS['TL_LANG'][$table]['pasteinto'][1], $row['id']), 'class="blink"');
        if ($row['id'] > 0) {
            $return = $disablePA ? Image::getHtml('pasteafter_.gif', '',
                          'class="blink"') . ' ' : '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&mode=1&pid=' . $row['id'] . (!is_array($arrClipboard['id']) ? '&id=' . $arrClipboard['id'] : '')) . '" title="' . specialchars(sprintf($GLOBALS['TL_LANG'][$table]['pasteafter'][1],
                          $row['id'])) . '" onclick="Backend.getScrollOffset();">' . $imagePasteAfter . '</a> ';
        }
        return $return . ($disablePI ? Image::getHtml('pasteinto_.gif', '',
                      'class="blink"') . ' ' : '<a href="' . $this->addToUrl('act=' . $arrClipboard['mode'] . '&mode=2&pid=' . $row['id'] . (!is_array($arrClipboard['id']) ? '&id=' . $arrClipboard['id'] : '')) . '" title="' . specialchars(sprintf($GLOBALS['TL_LANG'][$table]['pasteinto'][1],
...
<table cellpadding="0" cellspacing="0" width="100%" summary="">
    ...
    public function ondeleteCb()
    {
        ...
        if (Input::get('act') != 'deleteAll') {
            ...
            if ($this->restrictedUser) {
                $this->log('Datensatz mit ID ' . Input::get('id') . ' wurde von einem nicht authorisierten Benutzer versucht aus tl_gallery_creator_albums zu loeschen.',
                       __METHOD__, TL_ERROR);
    ...
    public function onloadCbGetGcCteElements()
    {
        ...
        foreach ($arrAlbumIds as $albumId) {
            ...
            $this->Database->prepare('UPDATE tl_gallery_creator_albums SET displ_alb_in_this_ce=? WHERE id=?')->execute($arrGcContentElements,
                   $albumId);
        }
    }
    ...
    public function onloadCbImportFromFilesystem()
    {
        ...
        if (strlen(trim($strMultiSRC))) {
            $this->Database->prepare('UPDATE tl_gallery_creator_albums SET preserve_filename=? WHERE id=?')->execute($blnPreserveFilename,
                   $intAlbumId);
    ...
    public function onloadCbSetUpPalettes()
    {
        ...
        if (Input::get('mode') == 'fileupload') {
            if ($this->User->gc_img_resolution == 'no_scaling') {
                $GLOBALS['TL_DCA']['tl_gallery_creator_albums']['palettes']['fileupload'] = str_replace(',img_quality',
                       '', $GLOBALS['TL_DCA']['tl_gallery_creator_albums']['palettes']['fileupload']);
            }
    ...
    public function optionsCbDisplAlbInThisContentElements()
    {
        ...
        $opt = array();
        while ($objDb->next()) {
            $opt[$objDb->id] = sprintf($GLOBALS['TL_LANG']['tl_gallery_creator_albums']['reference']['displ_alb_in_this_ce'],
                   $objDb->id, $objDb->title, $objDb->pagename);
        }
        return $opt;
    }
    ...
    public function optionsCbThumb()
    {
        ...
        $objDb = $this->Database->prepare('SELECT * FROM tl_gallery_creator_pictures WHERE pid=? ORDER BY id')->execute(Input::get('id'));
        $arrThumbId = array();
        while ($objDb->next()) {
            $oFile = FilesModel::findByUuid($objDb->uuid);
            if ($oFile !== null) {
                if (file_exists(TL_ROOT . '/' . $oFile->path)) {
                    $arrThumbId[$objDb->id] = basename($oFile->path);
                }
            }
        }
        ...
        if (count($arrSubalbums)) {
            foreach ($arrSubalbums as $albId) {
                $objPic = $this->Database->prepare('SELECT * FROM tl_gallery_creator_pictures WHERE pid=? ORDER BY id')->execute($albId);
                if ($objPic->numRows) {
                    ...
                    while ($objPic->next()) {
                        $oFile = FilesModel::findByUuid($objPic->uuid);
                        if ($oFile !== null) {
                            if (file_exists(TL_ROOT . '/' . $oFile->path)) {
                                $arrThumbId[$objPic->id] = basename($oFile->path);
                            }
                        }
                    }
                }
    ...
    public function saveCbDisplAlbInThisContentElements($varValue, DataContainer $dc)
    {
        ...
        while ($objContent->next()) {
            ...
            $this->Database->prepare('UPDATE tl_content SET gc_publish_albums=? WHERE id=?')->execute(serialize($arrPublAlbums),
                   $objContent->id);
            $this->log('A new version of record "tl_content.id=' . $objContent->id . '" has been created', __METHOD__,
                   GENERAL);
        }
        return $varValue;
    }
    ...
    public function saveCbSaveUploader($value)
    {
        ...
        $this->Database->prepare('UPDATE tl_user SET gc_be_uploader_template=? WHERE id=?')->execute($value,
               $this->User->id);
    }
