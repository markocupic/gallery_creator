...
$GLOBALS['TL_DCA']['tl_gallery_creator_albums'] = array(
	...
	'fields'      => array(
		...
		'thumb'                => array(
			...
			'inputType'        => 'select',
			'options_callback' => array('tl_gallery_creator_albums', 'optionsCbThumb'),
			'eval'             => array('doNotShow' => true, 'includeBlankOption' => true, 'nospace' => true, 'rgxp' => 'digit', 'maxlength' => 64, 'tl_class' => 'w50', 'submitOnChange' => true),
			'sql'              => "varchar(255) NOT NULL default ''"
		),
		'fileupload'           => array(
...
<table cellpadding="0" cellspacing="0" width="100%" summary="">
	...
	public function optionsCbThumb()
	{
		...
		if(count($arrSubalbums))
		{
			...
			{
				$objPic = $this->Database->prepare('SELECT * FROM tl_gallery_creator_pictures WHERE pid=? ORDER BY id')
										 ->execute($albId);
				if($objPic->numRows)
				{
