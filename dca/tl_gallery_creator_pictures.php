...
class tl_gallery_creator_pictures extends Backend
{
    ...
    public function buttonCbDeletePicture($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        return ($this->User->isAdmin || $this->User->id == $objImg->owner || true === $GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection']) ? '<a href="' . $this->addToUrl($href . '&id=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
    ...
    public function buttonCbEditImage($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        $objImg = $this->Database->prepare('SELECT owner FROM tl_gallery_creator_pictures WHERE id=?')->execute($row['id']);
        return ($this->User->isAdmin || $this->User->id == $objImg->owner || true === $GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection']) ? '<a href="' . $this->addToUrl($href . '&id=' . $row['id'],
                      true) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
                      $label) . '</a> ' : Image::getHtml(preg_replace('/\.gif$/i', '_.gif', $icon)) . ' ';
    }
    ...
    public function buttonCbRotateImage($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        return ($this->User->isAdmin || $this->User->id == $row['owner'] || true === $GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection']) ? '<a href="' . $this->addToUrl($href . '&imgId=' . $row['id']) . '" title="' . specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon,
                      $label) . '</a> ' : Image::getHtml($icon, $label);
    }
...
<div class="w50 easyExclude easyExcludeFN_picture" style="height:200px;">
	...
	<a href="' . $src . '" data-lightbox="gc_image_' . Input::get('id') . '"><img src="' . Image::get($src, '180',
               '180', 'crop') . '" width="100"></a>
</div>
		';
    }
    ...
    public function myParseBackendTemplate($strContent, $strTemplate)
    {
        ...
        if (Input::get('table') == 'tl_gallery_creator_pictures') {
            ...
            $strContent = str_replace('</fieldset>',
                   '<div class="clr" style="clear:both"><p> </p><!-- clearing Box --></div></fieldset>', $strContent);
        }
    ...
    public function toggleVisibility($intId, $blnVisible)
    {
        ...
        $objVersions->create();
        $this->log('A new version of record "tl_gallery_creator_pictures.id=' . $intId . '" has been created.',
