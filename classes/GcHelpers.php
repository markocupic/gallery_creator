...
class GcHelpers extends \System
{
	...
	public static function fileupload($intAlbumId, $arrFile)
	{
		...
		//move_uploaded_file
		if(\Files::getInstance()
				 ->move_uploaded_file($arrFile['tmp_name'], $strUploadPath . '/' . $strAlbumAlias . '/' . $arrFile['name'])
		)
		{
			...
			$return = array(
				'strFileSrc' => $strFileSrc, 'strAlbumAlias' => $strAlbumAlias, 'strFilename' => $arrFile['name']
			);
			return $return;
		}
		else
	...
	public static function generateUniqueFilename($strFilename)
	{
		...
		do
		{
			...
			$objImg = \Database::getInstance()
							   ->prepare('SELECT count(id) AS items FROM tl_gallery_creator_pictures WHERE name=?')
							   ->execute($strFilename);
			if($objImg->items < 1)
			{
	...
	public static function getAlbumInformationArray($intAlbumId, $objThis)
	{
		...
		$arrAlbum = array( //[int] Album-Id
		   ...
		   'event_tstamp'        => $objAlbum->date, 'date' => $objAlbum->date,
	...
	{
		//set pid if no is set
		/*
		$objFolder = \FilesModel::findAll();
		while($objFolder->next())
		{
			if(strlen($objFolder->pid) < 1)
			{
				$parentFolderPath = substr($objFolder->path, 0, -1 * (strlen($objFolder->name) + 1));
				$objParent = \FilesModel::findByPath($parentFolderPath);
				if($objParent !== NULL)
				{
					$parentUuid = $objParent->uuid;
					echo $parentUuid . '<br>';
					$objFolder->pid = $parentUuid;
					$objFolder->save();
				}
			}
		}
		*/
		...
		if(count($images))
		{
			...
			foreach($images as $image)
			{
				...
				{
					$strFilename = self::generateUniqueFilename($image['name']);
					...
					self::createNewImage($objAlb->id, $strDestSrc);
				}
				else
				{
	...
	public static function reviseTable($blnCleanDb = false)
	{
		...
		while($objAlb->next())
		{
			...
			{
				\Database::getInstance()->prepare('UPDATE tl_gallery_creator_albums SET pid=? WHERE id=?')
						 ->execute('0', $objAlb->id);
			}
		}
