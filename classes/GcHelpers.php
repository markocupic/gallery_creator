...
class GcHelpers extends \System
{
       ...
       public static function fileupload($intAlbumId, $arrFile)
       {
              ...
              if (\Files::getInstance()->move_uploaded_file($arrFile['tmp_name'], $strUploadPath . '/' . $strAlbumAlias . '/' . $arrFile['name']))
              {
                     ...
                     $return = array('strFileSrc' => $strFileSrc, 'strAlbumAlias' => $strAlbumAlias, 'strFilename' => $arrFile['name']);
                     return $return;
              }
              else
              {
       ...
       public static function generateUploader($intAlbumId, $uploader = 'be_gc_jumploader')
       {
              ...
              $arrJumploaderArchive = array(sprintf('%s/mediautil_z.jar', $pathToArchive), sprintf('%s/sanselan_z.jar', $pathToArchive), sprintf('%s/jumploader_z.jar', $pathToArchive), sprintf('%s/xfiledialog.jar', $pathToArchive),);
       ...
       public static function getAlbumInformationArray($intAlbumId, $objThis)
       {
              ...
              if (TL_MODE == 'FE')
              {
                     ...
                     $href = \Controller::generateFrontendUrl($objPage->row(), ($GLOBALS['TL_CONFIG']['useAutoItem'] ? '/%s##ceId##' : '/items/%s##ceId##'), $objPage->language);
              ...
              $arrAlbum = array( //[int] Album-Id
                     ...
                     'event_date' => \Date::parse($GLOBALS['TL_CONFIG']['dateFormat'], $objAlbum->date), //[string] Event-Location
                     ...
                     'comment' => strlen($objAlbum->comment) ? specialchars($objAlbum->comment) : NULL, 'caption' => strlen($objAlbum->comment) ? specialchars($objAlbum->comment) : NULL, //[int] Albumbesucher (Anzahl Klicks)
                     'visitors' => $objAlbum->visitors, //[string] Link zur Detailansicht
                     'href' => TL_MODE == 'FE' ? sprintf($href, $objAlbum->alias) : NULL, //[string] Inhalt fuer das title Attribut
                     'title' => $objAlbum->name . ' [' . ($objPics->numRows ? $objPics->numRows . ' ' . $GLOBALS['TL_LANG']['gallery_creator']['pictures'] : '') . ($objThis->gc_hierarchicalOutput && $objSubAlbums->countSubalbums > 0 ? ' ' . $GLOBALS['TL_LANG']['gallery_creator']['contains'] . ' ' . $objSubAlbums->countSubalbums . '  ' . $GLOBALS['TL_LANG']['gallery_creator']['subalbums'] . ']' : ']'), //[int] Anzahl Bilder im Album
                     'count' => $objPics->numRows, //[int] Anzahl Unteralben
                     'count_subalbums' => count(\GcHelpers::getAllSubalbums($objAlbum->id)), //[string] alt Attribut fuer das Vorschaubild
                     'alt' => $arrPreviewThumb['name'], //[string] Pfad zum Originalbild
                     'src' => TL_FILES_URL . $arrPreviewThumb['path'], //[string] Pfad zum Thumbnail
                     'thumb_src' => TL_FILES_URL . \Image::get($arrPreviewThumb['path'], $arrSize[0], $arrSize[1], $arrSize[2]), //[int] article id
                     'insert_article_pre' => $objAlbum->insert_article_pre ? $objAlbum->insert_article_pre : null, //[int] article id
                     'insert_article_post' => $objAlbum->insert_article_post ? $objAlbum->insert_article_post : null, //[string] css-Classname
       ...
       public static function getPictureInformationArray($intPictureId = null, $objThis)
       {
              ...
              $arrPicture = array( //[int] id picture_id
                     ...
                     'alt' => specialchars($objPicture->title ? $objPicture->title : $objPicture->name), //[string] title-attribut
                     'title' => specialchars($objPicture->title), //[string] Bildkommentar oder Bildbeschreibung
                     'comment' => specialchars($objPicture->comment), 'caption' => specialchars($objPicture->comment), //[string] path to media (video, picture, sound...)
                     'href' => TL_FILES_URL . $href, // single image url
                     'single_image_url' => \Controller::generateFrontendUrl($objPage->row(), ($GLOBALS['TL_CONFIG']['useAutoItem'] ? '/' : '/items/') . \Input::get('items') . '/img/' . $arrFile["filename"], $objPage->language), //[string] path to the image,
                     'image_src' => $strImageSrc, //[string] path to the other selected media
                     ...
                     'localMediaSRC' => $objPicture->localMediaSRC, //[string] Pfad zu einem benutzerdefinierten Thumbnail
                     'addCustomThumb' => $objPicture->addCustomThumb, //[string] Thumbnailquelle
                     'thumb_src' => isset($thumbSrc) ? TL_FILES_URL . $thumbSrc : '', //[array] Thumbnail-Ausmasse Array $arrSize[Breite, Hoehe, Methode]
                     ...
                     'image_height' => $arrFile["image_height"], //[int] das rel oder data-lightbox Attribut fuer das Anzeigen der Bilder in der Lightbox
                     'lightbox' => $objPage->outputFormat == 'xhtml' ? 'rel="lightbox[lb' . $objPicture->pid . ']"' : 'data-lightbox="lb' . $objPicture->pid . '"', //[int] Zeitstempel der letzten Aenderung
                     ...
                     'albuminfo' => $arrAlbumInfo, //[array] Array mit Bildinfos aus den meta-Angaben der Datei, gespeichert in tl_files.meta
                     'metaData' => $objThis->getMetaContent($objPicture->id), //[string] css-ID des Bildcontainers
                     'cssID' => $cssID[0] != '' ? $cssID[0] : '', //[string] css-Klasse des Bildcontainers
                     'cssClass' => $cssID[1] != '' ? $cssID[1] : '', //[bool] true, wenn es sich um ein Bild handelt, das nicht in files/gallery_creator_albums/albumname gespeichert ist
       ...
       {
              $objSubAlbums = \Database::getInstance()->prepare('SELECT * FROM tl_gallery_creator_albums WHERE pid=? AND published=? ORDER BY sorting ASC')->execute($intAlbumId, '1');
              $arrSubalbums = array();
              while ($objSubAlbums->next())
              {
                     $arrSubalbum = \GcHelpers::getAlbumInformationArray($objSubAlbums->id, $objThis);
