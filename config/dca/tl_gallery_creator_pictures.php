...
class tl_gallery_creator_pictures extends Backend
{
    ...
    public function __construct()
    {
        ...
        switch (Input::get('mode'))
        {
            ...
            case 'imagerotate' :
                ...
                $objPic = \MCupic\GalleryCreatorPicturesModel::findById(Input::get('imgId'));
        ...
        {
            $objPicture = \MCupic\GalleryCreatorPicturesModel::findByPk(Input::get('id'));
            if ($objPicture !== null)
            {
    ...
    public function onCutCb(DC_Table $dc)
    {
        ...
        $objSourceAlbum = \MCupic\GalleryCreatorAlbumsModel::findByPk($_SESSION['gallery_creator']['SOURCE_ALBUM_ID']);
        unset($_SESSION['gallery_creator']['SOURCE_ALBUM_ID']);

        // Get pictureToMoveObject
        $objPictureToMove = \MCupic\GalleryCreatorPicturesModel::findByPk(Input::get('id'));
        ...
        if (Input::get('mode') == '1')
        {
            ...
            $objTargetAlbum = \MCupic\GalleryCreatorPicturesModel::findByPk(Input::get('pid'))->getRelated('pid');
        }
        elseif (Input::get('mode') == '2')
        {
            // Paste on top
            $objTargetAlbum = \MCupic\GalleryCreatorAlbumsModel::findByPk(Input::get('pid'));
        }
    ...
    public function inputFieldCbGenerateImage()
    {
        ...
        $objImg = \MCupic\GalleryCreatorPicturesModel::findByPk(Input::get('id'));
...
<div class="w50 easyExclude easyExcludeFN_picture" style="height:auto;">
    ...
    public function inputFieldCbGenerateImageInformation(DataContainer $dc)
    {
        ...
        $objImg = \MCupic\GalleryCreatorPicturesModel::findByPk($dc->id);
    ...
    public function ondeleteCb(DC_Table $dc)
    {
        ...
        if ($objImg->owner == $this->User->id || $this->User->isAdmin || true === $GLOBALS['TL_CONFIG']['gc_disable_backend_edit_protection'])
        {
            ...
            if ($objImgNumRows->numRows)
            {
                ...
                $objAlbum = \MCupic\GalleryCreatorAlbumsModel::findByPk($objImg->pid);
    ...
    public function toggleVisibility($intId, $blnVisible)
    {
        ...
        $objPicture = \MCupic\GalleryCreatorPicturesModel::findByPk($intId);
